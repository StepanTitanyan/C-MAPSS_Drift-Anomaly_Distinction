...
def main():
    ...
    print("=" * 70)
    print("Stage C: Anomaly Detection — NLL vs URD (Stationarity) | Model: Gaussian GRU")
    print("=" * 70)
    ...
    model_path = os.path.join(cfg["paths"]["model_dir"], "gaussian_gru_best.pt")
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.to(device); model.eval()

    print("[2] Fitting scorers on validation data...")
    X_val, y_val, _ = create_windows(splits["val"], sensors, window_size=window_size, max_life_fraction=cfg["preprocessing"]["normal_life_fraction_threshold"])
    with torch.no_grad():
    ...
    nll_thresholds = nll_scorer.compute_thresholds(y_val, vm, vs, percentiles=[95.0, 97.5, 99.0])
    ...
    for eid in splits["test"]["unit_nr"].unique():
        ...
        if len(edf) <= window_size:
            continue
        engine_data_list.append({"engine_id": int(eid), "sensor_values": edf[sensors].values.copy(), "cycles": edf["time_cycles"].values.copy(), "life_fracs": edf["life_fraction"].values.copy()})
    ...
    injected = generator.generate_test_suite(engine_data_list=engine_data_list, magnitudes=cfg["synthetic_anomalies"]["magnitudes"], injection_positions=cfg["synthetic_anomalies"]["injection_positions"])
    print(f"   {len(injected)} injected trajectories")

    print("\n" + "=" * 70)
    print("EVALUATION: NLL-Only Scoring (Baseline)")
    print("=" * 70)
    nll_results = evaluate_method(model, injected, window_size, device, nll_score_fn, nll_thresholds)
    print(f"\n   Overall: ROC-AUC={nll_results['overall']['roc_auc']:.4f}  PR-AUC={nll_results['overall']['pr_auc']:.4f}")
    print(f"\n   Per Anomaly Type:")
    ...
    for pct, res in sorted(nll_results["threshold_results"].items()):
        ...
        print(f"\n   p{pct}: P={pm['precision']:.3f} R={pm['recall']:.3f} F1={pm['f1']:.3f} | EventR={em['event_recall']:.3f} Delay={em['mean_detection_delay']:.1f}")

    print("\n" + "=" * 70)
    print("EVALUATION: URD Combined Scoring (Deviation + Stationarity)")
    print("=" * 70)
    urd_results = evaluate_method(model, injected, window_size, device, urd_score_fn, urd_combined_thresholds)
    print(f"\n   Overall: ROC-AUC={urd_results['overall']['roc_auc']:.4f}  PR-AUC={urd_results['overall']['pr_auc']:.4f}")
    print(f"\n   Per Anomaly Type:")
    ...
    print("=" * 70)
    print(f"\n{'Anomaly Type':<22} {'NLL ROC':>9} {'URD ROC':>9} {'Change':>9} {'NLL PR':>9} {'URD PR':>9} {'Change':>9}")
    print("-" * 80)
    all_types = sorted(set(list(nll_results["per_type"].keys()) + list(urd_results["per_type"].keys())))
    for atype in all_types:
        ...
        print(f"{atype:<22} {nr:>9.4f} {ur:>9.4f} {ur-nr:>+9.4f} {np_:>9.4f} {up:>9.4f} {up-np_:>+9.4f}{marker}")
    no, uo = nll_results["overall"], urd_results["overall"]
    print(f"\n{'OVERALL':<22} {no['roc_auc']:>9.4f} {uo['roc_auc']:>9.4f} {uo['roc_auc']-no['roc_auc']:>+9.4f} {no['pr_auc']:>9.4f} {uo['pr_auc']:>9.4f} {uo['pr_auc']-no['pr_auc']:>+9.4f}")

    #Save
    res_dir = cfg["paths"]["results_dir"]
    os.makedirs(res_dir, exist_ok=True)
    save_data = {"nll_only": {"overall": nll_results["overall"], "per_type": nll_results["per_type"]}, "urd_stationarity": {"overall": urd_results["overall"], "per_type": urd_results["per_type"]}}
    ...
    with open(os.path.join(res_dir, "stage_c_head_to_head.csv"), "w", newline="") as f:
        ...
        w.writerow(["OVERALL", f"{no['roc_auc']:.4f}", f"{uo['roc_auc']:.4f}", f"{uo['roc_auc']-no['roc_auc']:+.4f}", f"{no['pr_auc']:.4f}", f"{uo['pr_auc']:.4f}", f"{uo['pr_auc']-no['pr_auc']:+.4f}"])
    ...
    with open(os.path.join(res_dir, "stage_c_threshold_metrics.csv"), "w", newline="") as f:
        ...
        for method, res in [("NLL", nll_results), ("URD", urd_results)]:
            for pct, r in sorted(res.get("threshold_results", {}).items()):
                ...
                w.writerow([method, pct, f"{pm['precision']:.4f}", f"{pm['recall']:.4f}", f"{pm['f1']:.4f}", f"{em['event_recall']:.4f}", f"{em['mean_detection_delay']:.1f}"])

    print(f"\n  CSVs saved to {res_dir}/")
    print(f"  Stage C Complete!")
