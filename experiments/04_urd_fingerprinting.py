"""
experiments/04_urd_fingerprinting.py — v3
==========================================
Stage E: URD Anomaly Fingerprinting

Reports BOTH:
  1. 5-class actionable taxonomy (main paper result)
  2. 9-class per-type breakdown (supplementary — shows within-category confusion)
...
def run_model(model, values, ws, device):
    ...
    X, y = [], []
    for i in range(T - ws):
        X.append(values[i:i+ws]); y.append(values[i+ws])
...
def extract_fp(model, traj, ws, device, nll_sc, urd_sc, aw=15):
    ...
    if not np.any(labels > 0): return None
    ...
    for i in range(len(labels)):
        if labels[i] > 0 and not in_ev: in_ev, start = True, i
        elif labels[i] == 0 and in_ev: events.append((start, i)); in_ev = False
    if in_ev: events.append((start, len(labels)))
    ...
    for s, e in events:
        ...
        row.extend([uf["deviation_at_peak"], uf["uncertainty_at_peak"],
                     uf["stationarity_at_peak"], uf["uncertainty_slope"],
                     uf["stationarity_max"], uf["du_ratio"],
                     uf["signed_deviation_mean"]])
        rows.append(row)
    return rows
...
def plot_signatures(profiles, names, save_path):
    ...
    colors = ["#E53935", "#FF7043", "#FFA726", "#66BB6A", "#42A5F5",
              "#7E57C2", "#26A69A", "#EF5350", "#78909C"]
...
def main():
    ...
    print("=" * 70)
    print("Stage E: URD Anomaly Fingerprinting")
    print("=" * 70)
    ...
    ti, vi, si = split_engines(df, cfg["preprocessing"]["train_ratio"],
        cfg["preprocessing"]["val_ratio"], cfg["preprocessing"]["test_ratio"],
    ...
    model.load_state_dict(torch.load(
        os.path.join(cfg["paths"]["model_dir"], "gaussian_lstm_best.pt"), map_location=device))
    model.to(device)
    ...
    urd_sc = URDScorer(fde_window=5)
    urd_sc.fit(yv, vm, vs)

    # Generate per-type data
    print("\n[1] Generating typed trajectories...")
    def prep(s): return [{"engine_id": int(eid),
        "sensor_values": s[s["unit_nr"]==eid].sort_values("time_cycles")[sensors].values.copy(),
        "cycles": s[s["unit_nr"]==eid].sort_values("time_cycles")["time_cycles"].values.copy(),
        "life_fracs": s[s["unit_nr"]==eid].sort_values("time_cycles")["life_fraction"].values.copy()}
        for eid in s["unit_nr"].unique() if len(s[s["unit_nr"]==eid]) > ws]
    ...
    def gen_typed(engines):
        typed = {}
        for at in atypes:
            typed[at] = ag.generate_test_suite(engines, magnitudes=[3.0, 5.0],
                injection_positions={"mid": 0.5}, anomaly_types=[at])
        for dt in dtypes:
            typed[dt] = dg.generate_test_suite(engines, rates=[0.03, 0.05],
                durations=[30, 60], injection_positions={"mid": 0.5}, drift_types=[dt])
        return typed

    val_typed, test_typed = gen_typed(ve), gen_typed(te)
    for tn, ts in val_typed.items():
        print(f"   Val {tn}: {len(ts)}")
    ...
    def extract_all(typed):
        ...
        return (np.array(feats) if feats else np.empty((0, len(URD_FEATURE_NAMES))),
                fine_labels, cat_labels)
    ...
    #D, U, S column indices
    d_col, u_col, s_col = 9, 10, 11

    # ===================================================================
    #EXPERIMENT 1: 5-CLASS CATEGORY CLASSIFICATION
    # ===================================================================
    print("\n" + "=" * 70)
    ...
    clf_5 = RandomForestClassifier(n_estimators=200, max_depth=10,
    ...
    for cn in cat_names:
        r = report_5[cn]
        print(f"   {cn:<22} {r['precision']:>8.3f} {r['recall']:>8.3f} "
              f"{r['f1-score']:>8.3f} {r['support']:>6.0f}")

    #URD profiles (5-class)
    ...
    # ===================================================================
    #EXPERIMENT 2: 9-CLASS PER-TYPE CLASSIFICATION
    # ===================================================================
    print("\n" + "=" * 70)
    ...
    clf_9 = RandomForestClassifier(n_estimators=200, max_depth=10,
        class_weight="balanced", random_state=42, n_jobs=-1)
    ...
    print(f"\n   {'Type':<22} {'Prec':>8} {'Recall':>8} {'F1':>8} {'N':>6}")
    print("   " + "-" * 52)
    for fn in fine_names:
        r = report_9[fn]
        print(f"   {fn:<22} {r['precision']:>8.3f} {r['recall']:>8.3f} "
              f"{r['f1-score']:>8.3f} {r['support']:>6.0f}")

    #URD profiles (9-class)
    print(f"\n   URD Signature Profiles (9-class):")
    profiles_9 = []
    for fn in fine_names:
        ...
        if mask.sum() > 0:
            p = [X_te[mask, d_col].mean(), X_te[mask, u_col].mean(), X_te[mask, s_col].mean()]
            profiles_9.append(p)
            print(f"   {fn:<22}: D={p[0]:>8.2f}  U={p[1]:>8.3f}  S={p[2]:>8.2f}")
        else:
            profiles_9.append([0, 0, 0])

    # ===================================================================
    #EXPERIMENT 3: SPIKE vs DROP (signed_deviation ablation)
    # ===================================================================
    print("\n" + "=" * 70)
    ...
    if sd_mask_tr.sum() > 0 and sd_mask_te.sum() > 0:
        ...
        sd_names = le_sd.classes_.tolist()

        clf_sd = RandomForestClassifier(n_estimators=100, max_depth=6,
            class_weight="balanced", random_state=42)
        clf_sd.fit(X_sd_tr, y_sd_tr_e)
        pred_sd = clf_sd.predict(X_sd_te)
        acc_sd = float(np.mean(pred_sd == y_sd_te_e))

        print(f"\n   Spike vs Drop Accuracy: {acc_sd:.3f}")
        report_sd = classification_report(y_sd_te_e, pred_sd,
            target_names=sd_names, output_dict=True)
        for sn in sd_names:
            r = report_sd[sn]
            print(f"   {sn}: P={r['precision']:.3f} R={r['recall']:.3f} F1={r['f1-score']:.3f}")

        # Feature ranking
        sd_imp = dict(zip(URD_FEATURE_NAMES, clf_sd.feature_importances_))
        signed_rank = sorted(sd_imp.items(), key=lambda x: -x[1])
        print(f"\n   Feature ranking:")
        for i, (n, v) in enumerate(signed_rank[:5]):
            marker = " ← SIGNED DEVIATION" if n == "signed_deviation_mean" else ""
            print(f"   #{i+1}: {n} = {v:.4f}{marker}")

        # Ablation: without signed_deviation
        signed_idx = URD_FEATURE_NAMES.index("signed_deviation_mean")
        clf_nosign = RandomForestClassifier(n_estimators=100, max_depth=6,
            class_weight="balanced", random_state=42)
        clf_nosign.fit(np.delete(X_sd_tr, signed_idx, axis=1), y_sd_tr_e)
        acc_nosign = float(np.mean(
            clf_nosign.predict(np.delete(X_sd_te, signed_idx, axis=1)) == y_sd_te_e))
    ...
    X_tr_9 = fsc_9.fit_transform(X_tr[:, :9])
    X_te_9 = fsc_9.transform(X_te[:, :9])
    clf_abl = RandomForestClassifier(n_estimators=200, max_depth=10,
        class_weight="balanced", random_state=42, n_jobs=-1)
    ...
    plot_cm(cm_5, cat_names, f"5-Class Fingerprinting (Acc: {acc_5:.1%})",
            os.path.join(fig_dir, "fingerprint_5class_cm.png"))
    plot_cm(cm_9, fine_names, f"9-Class Per-Type (Acc: {acc_9:.1%})",
            os.path.join(fig_dir, "fingerprint_9class_cm.png"))
    plot_signatures(profiles_5, cat_names,
                    os.path.join(fig_dir, "urd_profiles_5class.png"))
    plot_signatures(profiles_9, fine_names,
                    os.path.join(fig_dir, "urd_profiles_9class.png"))

    save_data = {
        "five_class": {"accuracy": acc_5, "report": report_5,
            "profiles": {n: {"D": p[0], "U": p[1], "S": p[2]}
                         for n, p in zip(cat_names, profiles_5)}},
        "nine_class": {"accuracy": acc_9, "report": report_9,
            "profiles": {n: {"D": p[0], "U": p[1], "S": p[2]}
                         for n, p in zip(fine_names, profiles_9)}},
        "spike_vs_drop": {"accuracy_with_signed": acc_sd,
                          "accuracy_without_signed": acc_nosign},
    ...
    with open(os.path.join(res_dir, "stage_e_5class.csv"), "w", newline="") as f:
        ...
        for i, cn in enumerate(cat_names):
            r = report_5[cn]
            p = profiles_5[i]
            w.writerow([cn, f"{r['precision']:.4f}", f"{r['recall']:.4f}",
    ...
    with open(os.path.join(res_dir, "stage_e_9class.csv"), "w", newline="") as f:
        ...
        for i, fn in enumerate(fine_names):
            r = report_9[fn]
            p = profiles_9[i]
            w.writerow([fn, f"{r['precision']:.4f}", f"{r['recall']:.4f}",
                        f"{r['f1-score']:.4f}", int(r['support']),
                        f"{p[0]:.4f}", f"{p[1]:.4f}", f"{p[2]:.4f}"])
